  ...
  return <>
    {isOpen ? <div className="language-picker-choices">
      {supportedLanguages.map(lang => {
        return <div className={"clickable language-list-item"+ (i18n.language == lang ? " active":"")} onClick={() => i18n.changeLanguage(lang)}>
          <div className="flag" style={{
            backgroundImage: `url("${flags[lang]}")`
          }}>
          </div>
          <span className="language-list-item-name">{supportedLanguagesFullNames[lang]}</span>
        </div>
      })}
    </div> : ''}
